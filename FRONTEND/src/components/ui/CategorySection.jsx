...
const CategorySection = forwardRef((props, ref) => {
  ...
  useEffect(() => {
    if (scrollRef.current) {
      scrollRef.current.style.animationPlayState = isHovered
        ? "paused"
        : "running";
    }
  }, [isHovered]);
    ...
    <>
      <div className="py-8 overflow-hidden">
        ...
        <div className="relative w-full overflow-hidden">
          <div
            ref={scrollRef}
            className="loop-scroll-container"
          ...
          >
            {[...categories, ...categories].map((cat, index) => (
              <div
              key={index}
              className="min-w-[180px] h-[180px] snap-start relative group rounded-2xl overflow-hidden border-5 border-white shadow-md hover:shadow-xl transition duration-300 cursor-pointer mx-2"
            >
              <img
                src={cat.image}
                alt={cat.name}
                className="w-full h-full object-cover z-20 transition-transform duration-300 ease-in-out group-hover:scale-190"
